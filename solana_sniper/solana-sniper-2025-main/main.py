...
def bot():
    ...
    now = datetime.now()
    cprint(f'🌙 MOON DEV live bot running at {now}', 'white', 'on_green')
    ...
    cprint("\n🔍 MOON DEV: Checking SOL Balance", 'white', 'on_blue')
    
    while retry_count < 3 and (sol_amount is None or sol_value is None):
        retry_count += 1
        cprint(f"\n💫 MOON DEV: SOL Balance Check Attempt {retry_count}/3", 'white', 'on_cyan')
    ...
    if float(sol_amount) < 0.005:
        cprint(f"🚨 MOON DEV: SOL BALANCE CRITICAL! Only {sol_amount} SOL remaining", 'white', 'on_red')
    ...
    cprint("\n💰 MOON DEV SOL BALANCE INFO:", 'white', 'on_light_blue')
...
####### 🚨🚨🚨🚨🚨🚨🚨🚨🚨 BELOW SECTION I MARKED OUT CUZ IM HANDLING RISK AND TAKE PROFIT IN COPY BOT
    ...
    #         cprint(f'🌙 MOON DEV: Winning Position - Token: {token_mint_address}', 'white', 'on_green')
    #         n.pnl_close(token_mint_address)
    #         #print('i am handling pnl close in copy bot so marked the above out for now..')
    #     cprint('✨ MOON DEV: Done closing winning positions...', 'white', 'on_magenta')
    ...
    #         cprint(f'🚫 MOON DEV: Skipping trade for {token_mint_address} (in DO_NOT_TRADE_LIST)', 'white', 'on_red')
    #         continue
    #     if token_mint_address != USDC_CA:
    #         n.pnl_close(token_mint_address)
           
    # cprint('🌙 MOON DEV: Done closing losing positions.. keep swimming ❤️ 🙏', 'white', 'on_magenta')


####### 🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABOVE SECTION I MARKED OUT CUZ IM HANDLING RISK AND TAKE PROFIT IN COPY BOT

    # Run token scan every time
    cprint(f'🔍 MOON DEV: Running token scan...', 'white', 'on_cyan')
    scan_bot()  # Run the scan_bot from get_new_tokens.py
        
    try:
        df = pd.read_csv(FINAL_SORTED_CSV)
    except Exception as e:
        cprint(f"❌ MOON DEV: Error reading {FINAL_SORTED_CSV}: {str(e)}", 'white', 'on_red')
        return
...
# 🍀 THIS IS WHERE THE BUYING STARTS
    for index, row in df.iterrows():
        ...
        if usdc_holdings > USDC_SIZE:
            cprint(f'💰 MOON DEV: USDC Balance {usdc_holdings} > {USDC_SIZE}, opening position...', 'white', 'on_blue')
            cprint(f'📝 Token Address: {token_mint_address}', 'white', 'on_blue')
            n.open_position(token_mint_address)
        else:
            cprint(f'⚠️ MOON DEV: Insufficient USDC ({usdc_holdings}), skipping position', 'white', 'on_red')
    
    time.sleep(5)

bot()
cprint('🌙 MOON DEV: Done with 1st run, now looping...', 'white', 'on_green')
...
while True:
    try:
        ...
        cprint('❌ MOON DEV: Connection error!', 'white', 'on_red')
