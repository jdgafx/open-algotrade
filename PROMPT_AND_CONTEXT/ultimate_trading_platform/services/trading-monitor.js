...
class TradingMonitor {
    ...
    async checkMonitoringRules(metrics) {
        const activeRules = Array.from(this.monitoringRules.values()).filter(rule => rule.enabled);
        
        for (const rule of activeRules) {
            try {
                const conditionMet = await this.evaluateRuleCondition(rule.condition, metrics);
                
                if (conditionMet && !this.alerts.has(rule.id)) {
                    await this.triggerAlert(rule, metrics);
                } else if (!conditionMet && this.alerts.has(rule.id)) {
