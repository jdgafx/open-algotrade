 ...
 * Features 500+ AI models for comprehensive trading analysis
 */

export default class AIOptimizer {
    constructor() {
        this.models = {
            fast: 'gpt-4o-mini',
    ...
    /**
     * Start the background AI optimization loop
     */
    startOptimization() {
    ...
    /**
     * Stop the background AI optimization loop
     */
    stopOptimization() {
        if (this.optimizationInterval) {
            clearInterval(this.optimizationInterval);
            console.log('🤖 AI optimization loop stopped');
        }
    }

    /**
     * Core AI model calling method with fallback support
     */
    async callAIModel(prompt, model, options = {}) {
        ...
        try {
            const response = await puter.ai.chat(model, prompt, defaultOptions);
            return typeof response === 'object' ? JSON.stringify(response) : response;
        } catch (error) {
            console.warn(`Model ${model} failed, trying fallbacks:`, error.message);

            // Try fallback models
            const fallbacks = this.modelFallbacks[model] || [];
            for (const fallbackModel of fallbacks) {
                try {
                    console.log(`Trying fallback model: ${fallbackModel}`);
                    const response = await puter.ai.chat(fallbackModel, prompt, defaultOptions);
                    return response;
                } catch (fallbackError) {
     ...
     */
    parseAIResponse(response, type = 'text') {
        try {
    ...
    async selectOptimalStrategies(marketConditions) {
        try {
            ...
            const aiResponse = await this.callAIModel(prompt, this.models.powerful, {
                max_tokens: 2000,
                temperature: 0.3
            });
            ...
            return ranked.map(rec => ({
                ...rec,
                action: rec.action || (rec.score > 0.7 ? 'buy' : 'hold'),
                signal: rec.signal || (rec.score > 0.7 ? 'buy' : 'hold'),
                allocation: rec.allocation || 0.2
            }));
        } catch (error) {
    ...
    async generateTradingSignal(symbol, marketData, strategy) {
        try {
            ...
            const signal = await this.callAIModel(prompt, this.models.balanced, {
                max_tokens: 1000,
                temperature: 0.4
            });
            ...
            const finalSignal = {
                ...parsedSignal,
                action: parsedSignal.action || parsedSignal.signal || 'hold'
            };

            // Store signal for performance tracking
            await this.storeTradingSignal(symbol, finalSignal);
    ...
    async performRiskAnalysis(portfolio, marketData) {
        try {
            ...
            const analysis = await this.callAIModel(prompt, this.models.powerful, {
                max_tokens: 2000,
                temperature: 0.2
            });
    ...
    async backtestStrategy(strategy, historicalData) {
        try {
            ...
            const backtestResults = await this.callAIModel(prompt, this.models.powerful, {
                max_tokens: 2000,
                temperature: 0.1
            });
    ...
    async optimizePortfolio(targetStrategies, constraints) {
        try {
            ...
            const optimization = await this.callAIModel(prompt, this.models.powerful, {
                max_tokens: 2000,
                temperature: 0.3
            });
    ...
    async adaptStrategy(strategy, marketChange) {
        try {
            ...
            const adaptedStrategy = await this.callAIModel(adaptationPrompt, this.models.balanced, {
                max_tokens: 2000,
                temperature: 0.5
            });
    ...
    async getOptimizationRecommendations(strategy, performance, marketConditions) {
        try {
            ...
            const recommendations = await this.callAIModel(prompt, this.models.balanced, {
                max_tokens: 2000,
                temperature: 0.4
            });
    ...
    extractSentiment(text) {
        if (typeof text === 'object') return text.sentiment || 'neutral';
        const match = text.match(/sentiment[:\s]*(\w+)/i);
        return match ? match[1] : 'neutral';
    }

    extractVolatility(text) {
        if (typeof text === 'object') return text.volatility || 'medium';
        const match = text.match(/volatility[:\s]*(\w+)/i);
        return match ? match[1] : 'medium';
    }

    extractVolume(text) {
        if (typeof text === 'object') return text.volume || 'normal';
        const match = text.match(/volume[:\s]*(\w+)/i);
        return match ? match[1] : 'normal';
    }

    extractPatterns(text) {
        if (typeof text === 'object') return text.patterns || 'mixed';
        const match = text.match(/patterns?[:\s]*([^.]+)/i);
        return match ? match[1].trim() : 'mixed';
    }

    extractRisk(text) {
        if (typeof text === 'object') return text.riskLevel || text.risk || 'medium';
        const match = text.match(/risk[:\s]*(\w+)/i);
        return match ? match[1] : 'medium';
    }

    extractTrend(text) {
        if (typeof text === 'object') return text.trend || 'neutral';
        const match = text.match(/trend[:\s]*(\w+)/i);
        return match ? match[1] : 'neutral';
    }
