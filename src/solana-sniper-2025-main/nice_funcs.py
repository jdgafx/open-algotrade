...
def get_sol_balance(wallet_address):
    ...
    try:
        ...
        cprint(f"⚠️ MOON DEV: Error getting SOL balance: {str(e)}", 'white', 'on_red')
        return None, None
...
def get_names(df):
    ...
    for index, row in df.iterrows():
        ...
        # Extract the token name using the 'name' key from the token_data
        token_name = token_data.get('name', 'N/A')  # Use 'N/A' if name isn't provided
        cprint(f'🌙 MOON DEV: Token {token_name} at address: {token_mint_address}', 'white', 'on_cyan')
        names.append(token_name)
...
def fetch_wallet_holdings_og(address):
    ...
    if response.status_code == 200:
        ...
        if 'data' in json_response and 'items' in json_response['data']:
            ...
            cprint("❌ MOON DEV: No data available in the response.", 'white', 'on_red')

    else:
        cprint(f"❌ MOON DEV: Failed to retrieve token list for address: {address}", 'white', 'on_magenta')
        time.sleep(10)
    ...
    if not df.empty:
        ...
        print('')
        print(df2.head(20))
        cprint(f'💰 MOON DEV: Current Portfolio Value: ${round(df2["USD Value"].sum(),2)}', 'white', 'on_green')
        print(' ')
        time.sleep(7)
    else:
        cprint("❌ MOON DEV: No wallet holdings to display.", 'white', 'on_red')
        time.sleep(30)
    ...
    while True:
        try:
            ...
            if 'swapTransaction' not in txRes:
                swap_error_count += 1
                cprint(f'🚨 MOON DEV: SwapTransaction error #{swap_error_count}/50 for token {token[-4:]}', 'white', 'on_red')
                
                if swap_error_count >= max_retries:
                    cprint(f'💀 MOON DEV: Blacklisting token {token[-4:]} after {max_retries} swap transaction errors!', 'white', 'on_red')
            ...
            cprint(f"🌟 MOON DEV: Transaction successful! https://solscan.io/tx/{str(txId)}", 'white', 'on_green')
            return True
            
        except requests.exceptions.RequestException as e:
            cprint(f"🔄 MOON DEV: Request failed: {e}", 'white', 'on_red')
            time.sleep(5)
        except Exception as e:
            cprint(f"⚠️ MOON DEV: An error occurred: {e}", 'white', 'on_red')
            time.sleep(5)
...
def open_position(token_mint_address):
    cprint(f'🌙 MOON DEV: Opening position for token: {token_mint_address}', 'white', 'on_blue')
    ...
    try:
        with open(PERMANENT_BLACKLIST, 'r') as f:
            ...
            if token_mint_address in blacklisted:
                cprint(f'⛔ MOON DEV: Token {token_mint_address} is permanently blacklisted, skipping', 'white', 'on_red')
                return
    ...
    if initial_balance > 0:
        cprint(f'⚠️ MOON DEV: Already have position in {token_mint_address}, adding to closed positions', 'white', 'on_red')
        with open(CLOSED_POSITIONS_TXT, 'a') as f:
    ...
    with open(CLOSED_POSITIONS_TXT, 'r') as f:
        ...
        if token_mint_address in closed_positions:
            cprint(f'⚠️ MOON DEV: Token {token_mint_address} in closed positions, skipping', 'white', 'on_red')
            return

    buying_df = pd.read_csv(READY_TO_BUY_CSV)
    if token_mint_address not in buying_df['address'].values:
        cprint(f'⚠️ MOON DEV: Token {token_mint_address} not in buying list, skipping', 'white', 'on_red')
        return
    ...
    price = ask_bid(token_mint_address)
    if not price:
        cprint(f'⚠️ MOON DEV: Could not get price for {token_mint_address}, skipping', 'white', 'on_red')
        return
    ...
    try:
        ...
        for i in range(orders_per_open):
            cprint(f'🎯 MOON DEV: Attempting order {i+1}/{orders_per_open} for {token_mint_address}', 'white', 'on_blue')
            
            # Check the return value from market_buy
            if not market_buy(token_mint_address, size_needed):
                cprint(f'❌ MOON DEV: Market buy failed for {token_mint_address}, token may be blacklisted', 'white', 'on_red')
                return
            ...
            if current_balance > 0:
                cprint(f'✅ MOON DEV: Position opened! Balance: {current_balance}', 'white', 'on_green')
        ...
        cprint(f'❌ MOON DEV: Order failed: {str(e)}', 'white', 'on_red')
        time.sleep(30)
        try:
            for i in range(orders_per_open):
                if not market_buy(token_mint_address, size_needed):
                    cprint(f'❌ MOON DEV: Market buy failed on retry for {token_mint_address}', 'white', 'on_red')
                    return
                ...
                current_balance = get_position(token_mint_address)
                if current_balance > 0:
                    cprint(f'✅ MOON DEV: Position opened on retry! Balance: {current_balance}', 'white', 'on_green')
                    with open(CLOSED_POSITIONS_TXT, 'a') as f:
                        f.write(f'{token_mint_address}\n')
                    return
                    
        except:
            cprint('❌ MOON DEV: Order failed again, logging to closed positions', 'white', 'on_red')
            with open(CLOSED_POSITIONS_TXT, 'a') as f:
                f.write(f'{token_mint_address}\n')
            return
    ...
    if final_balance > 0:
        cprint(f'✅ MOON DEV: Final position check - Balance: {final_balance}', 'white', 'on_green')
        with open(CLOSED_POSITIONS_TXT, 'a') as f:
            f.write(f'{token_mint_address}\n')
    else:
        cprint(f'❌ MOON DEV: No position opened for {token_mint_address}', 'white', 'on_red')
