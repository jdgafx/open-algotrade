...
class EnhancedMarketMaker:
    ...
    async def iteration(self):
        """Single iteration of the market making logic"""
    ...
    async def _risk_management_checks(self):
        ...
        if self.current_position:
            position_cost = abs(
                self.current_position.size * self.current_position.entry_price
            )

            if position_cost > self.config.risk_limit:
                logger.error(
                    f"🚨 EMERGENCY: Position cost {position_cost} exceeds risk limit {self.config.risk_limit}"
                )
    ...
    async def _calculate_atr(self) -> Decimal:
        ...
        try:
            ...
            ohlcv_data = await self.client.get_ohlcv(
                self.config.symbol, "5m", self.config.atr_period + 20
            )
    ...
    async def _volatility_filter(self, atr: Decimal, market_data: MarketData) -> bool:
        ...
        try:
            ...
            if (
                current_price >= recent_highs.max()
                or current_price <= recent_lows.min()
            ):
                return False
    ...
    async def _place_entry_orders(self, market_data: MarketData):
        ...
        try:
            ...
            buy_prices = [
                market_data.bid
                * (Decimal("1") - Decimal("0.0009")),  # Level 1: 0.09% below bid
                market_data.bid
                * (Decimal("1") - Decimal("0.003")),  # Level 2: 0.3% below bid
                market_data.bid
                * (Decimal("1") - Decimal("0.005")),  # Level 3: 0.5% below bid
            ]

            # Sell orders (above current price)
            sell_prices = [
                market_data.ask
                * (Decimal("1") + Decimal("0.0009")),  # Level 1: 0.09% above ask
                market_data.ask
                * (Decimal("1") + Decimal("0.003")),  # Level 2: 0.3% above ask
                market_data.ask
                * (Decimal("1") + Decimal("0.005")),  # Level 3: 0.5% above ask
            ]

            # Calculate sizes for each level
            for i, (buy_price, sell_price) in enumerate(zip(buy_prices, sell_prices)):
                size = self.config.base_size * Decimal(
                    str(self.config.size_distribution[i])
                )
        ...
        try:
            ...
            if self.current_position.side == "long":
                ...
                exit_prices = [
                    ...
                    * (Decimal("1") + Decimal("0.004")),  # 0.4% profit
                    self.current_position.entry_price
                    * (Decimal("1") + Decimal("0.006")),  # 0.6% profit
                    self.current_position.entry_price
                    * (Decimal("1") + Decimal("0.008")),  # 0.8% profit
                ]
                side = "sell"
            else:
            ...
            logger.info(
                f"Placed exit orders for {side} position of {abs(self.current_position.size)}"
            )
    ...
    async def _check_exit_conditions(self):
        ...
        try:
            ...
            if time_in_position > self.config.time_limit_minutes * 60:
                logger.info(
                    f"Time limit reached ({self.config.time_limit_minutes} minutes), exiting position"
                )
                await self._emergency_close()
                return
            ...
            if pnl_percentage > 0.5:  # 0.5% profit target
                logger.info(
                    f"Profit target reached ({pnl_percentage:.2f}%), exiting position"
                )
                await self._emergency_close()
                return
    ...
    async def _check_kill_switch_conditions(self):
        ...
        try:
            ...
            if pnl_percentage < -2.0:  # 2% loss triggers kill switch
                logger.error(
                    f"🚨 KILL SWITCH: Extreme loss detected ({pnl_percentage:.2f}%)"
                )
            ...
            if abs(self.current_position.size) > self.config.max_position_size:
                logger.error(
                    f"🚨 KILL SWITCH: Position size {self.current_position.size} exceeds limit {self.config.max_position_size}"
                )
                await self.client.kill_switch(self.config.symbol)
                return
    ...
    def get_performance_stats(self) -> Dict:
        ...
        return {
            ...
            "atr_average": float(np.mean([float(atr) for atr in self.atr_values[-20:]]))
            if self.atr_values
            else 0,
        }
