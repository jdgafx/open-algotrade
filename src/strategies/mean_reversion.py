...
class MeanReversionTrader:
    ...
    def __init__(self, client: HyperliquidClient, config: MeanReversionConfig):
        ...
        logger.info(
            f"Initialized Mean Reversion Trader for {len(config.symbols)} symbols"
        )

    async def start(self):
        """Start the mean reversion trading bot"""
        logger.info(
            f"📊 Starting Mean Reversion Trader for {len(self.config.symbols)} symbols"
        )
    ...
    async def iteration(self):
        ...
        try:
            ...
            logger.error(f"Error in mean reversion iteration: {e}")
        ...
        try:
            ...
            for symbol in self.config.symbols:
                ...
                self.market_stats[symbol] = {
                    ...
                    "ask": market_data.ask,
                    "spread_percentage": float(
                        (market_data.ask - market_data.bid) / current_price * 100
                    ),
    ...
    async def _calculate_market_statistics(self):
        ...
        try:
            for symbol in self.config.symbols:
                ...
                recent_prices = [
                    float(price)
                    for _, price in self.price_history[symbol][
                ...
                for i in range(self.config.sma_period, len(recent_prices)):
                    ...
                    sma = (
                        sma_values[i - self.config.sma_period]
                        if i - self.config.sma_period < len(sma_values)
                        else current_sma
                    )
                    deviation = price - sma
    ...
    async def _calculate_correlation_matrix(self):
        ...
        try:
            ...
            for symbol in symbols:
                if (
                    symbol not in self.price_history
                    or len(self.price_history[symbol]) < 50
                ):
                    continue
        ...
        try:
            ...
            for symbol in self.config.symbols:
                ...
                if z_score <= -self.config.entry_threshold_deviation:
                    ...
                    confidence = min(
                        1.0, abs(z_score) / self.config.entry_threshold_deviation
                    )
        ...
        try:
            ...
            for signal in signals:
                ...
                for existing_symbol in current_positions:
                    if (
                        symbol in self.correlation_matrix
                        and existing_symbol in self.correlation_matrix[symbol]
                        and abs(self.correlation_matrix[symbol][existing_symbol])
                        > self.config.correlation_threshold
                    ):
                        correlation_ok = False
                        logger.debug(
                            f"Skipping {symbol} - high correlation with {existing_symbol}"
                        )
                        break
    ...
    async def _execute_signals(self, signals: List[MeanReversionSignal]):
        ...
        try:
            for signal in signals:
                logger.info(
                    f"🎯 Executing mean reversion signal: {signal.direction} {signal.symbol}"
                )
                logger.info(
                    f"   Z-Score: {signal.z_score:.2f}, Deviation: {signal.deviation:.2f}σ, Confidence: {signal.confidence:.2f}"
                )
    ...
    async def _calculate_position_size(self, signal: MeanReversionSignal) -> Decimal:
        ...
        try:
            ...
            confidence_adjusted_size = self.config.size_per_position * Decimal(
            ...
            if signal.volatility > 0:
                volatility_adjustment = min(
                    2.0, self.config.volatility_threshold / signal.volatility
                )
            else:
    ...
    async def _enter_long_position(self, signal: MeanReversionSignal, size: Decimal):
        ...
        try:
            ...
            if order_response.get("status") == "ok":
                logger.info(
                    f"✅ Long mean reversion order placed: {size} {signal.symbol} @ {signal.entry_price}"
                )
            else:
    ...
    async def _enter_short_position(self, signal: MeanReversionSignal, size: Decimal):
        ...
        try:
            ...
            if order_response.get("status") == "ok":
                logger.info(
                    f"✅ Short mean reversion order placed: {size} {signal.symbol} @ {signal.entry_price}"
                )
            else:
    ...
    async def _check_exit_conditions(self, symbol: str, position: Position):
        ...
        try:
            ...
            if exit_triggered:
                logger.info(
                    f"📤 Exiting {symbol} position: {exit_reason}, Z-Score: {current_z_score:.2f}, PnL: {pnl_percentage:.2f}%"
                )
