...
def get_ohlcv_data(address):
    ...
    if response.status_code != 200:
        cprint(f"🚨 MOON DEV: Failed to get OHLCV data for {address}", 'red')
        return pd.DataFrame()
    ...
    cprint("\n📊 MOON DEV: Latest OHLCV Data for Token 🌙", 'cyan', attrs=['bold'])
    print("\nLast 5 candles:")
...
def check_ohlcv_conditions(ohlcv_df, trend_analysis, address):
    ...
    if trend_analysis['num_bars'] > max_amount_of_bars_before_dropping:
        cprint(f"🌙 MOON DEV: Dropping {address} - Too many bars ({trend_analysis['num_bars']} > {max_amount_of_bars_before_dropping})", 'yellow')
        return False

    # Check price vs average close
    if only_keep_if_above_avg_close and not trend_analysis['price_above_avg_close']:
        cprint(f"🌙 MOON DEV: Dropping {address} - Price below average close", 'yellow')
        return False
    ...
    if len(ohlcv_df) >= 30:
        ...
        if any(conditions):
            cprint(f"🚀 MOON DEV: {address} PASSED OHLCV FILTERS! 🌙", 'white', 'on_green', attrs=['bold'])
            return True
            
    cprint(f"🌙 MOON DEV: Dropping {address} - Failed technical conditions", 'yellow')
    return False
...
def token_overview(address, MAX_SELL_PERCENTAGE, MIN_TRADES_LAST_HOUR, MIN_UNQ_WALLETS2hr, MIN_VIEW24h, MIN_LIQUIDITY):
    ...
    cprint(f"🚀 MOON DEV FOUND A GEM! 🌙", 'white', 'on_green', attrs=['bold'])
...
def add_to_blacklist(token_address, reason):
    ...
    cprint(f'🌙 MOON DEV: Adding {token_address} to blacklist. Reason: {reason}', 'white', 'on_red')
...
def scan_bot():
    ...
    cprint('🌙 MOON DEV: Starting token scan...', 'white', 'on_cyan')
    print('\n🚀 Fetching tokens from Jupiter API...')

    # Get all new tokens first
    all_tokens_df = get_jupiter_tokens()
    if all_tokens_df is None or all_tokens_df.empty:
        cprint('❌ MOON DEV: No tokens found from Jupiter API', 'white', 'on_red')
        return
    ...
    cprint(f'🌙 MOON DEV: Found {len(time_filtered_df)} tokens after removing blacklisted ones', 'white', 'on_cyan')
    ...
    for index, row in time_filtered_df.iterrows():
        ...
        cprint(f'\n🔍 MOON DEV: Checking token {token_address}', 'white', 'on_blue')
